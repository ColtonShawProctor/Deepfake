#!/usr/bin/env python3
"""
Train deepfake detection models with large synthetic dataset

This script trains the models using the larger synthetic dataset we generated,
providing much better training data than the original 5 test samples.
"""
...
class LargeSyntheticDataset(Dataset):
    """Dataset using the large synthetic training data"""
    
    def __init__(self, training_data_dir: str = "training_data", transform=None):
        self.training_data_dir = Path(training_data_dir)
        self.transform = transform or self._get_default_transform()
        
        # Load metadata
        with open(self.training_data_dir / "training_metadata.json", 'r') as f:
        ...
        logger.info(f"Loaded {len(self.samples)} samples from large synthetic dataset")
...
class AdvancedModelTrainer:
    """Advanced trainer for deepfake detection models with large dataset"""
    ...
    def train(self, dataset: LargeSyntheticDataset, epochs: int = 50, lr: float = 0.0001, 
        ...
        optimizer = optim.Adam(self.model.model.parameters(), lr=lr, weight_decay=1e-4)
        ...
        history_path = self.models_dir / f"{self.model_name}_training_history.json"
    ...
    def evaluate(self, dataset: LargeSyntheticDataset) -> Dict[str, float]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train deepfake detection models with large dataset")
    ...
    parser.add_argument("--training-data-dir", default="training_data", help="Training data directory")
    parser.add_argument("--models-dir", default="models", help="Models directory")
    
    args = parser.parse_args()
    
    # Load dataset
    dataset = LargeSyntheticDataset(args.training_data_dir)
    ...
    for model_name in models:
        ...
        trainer = AdvancedModelTrainer(model_name, args.models_dir)
